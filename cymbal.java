...
public class cymbal
{
  ...
  static final int MAX_CASE					= 101;
  static final int NOT_CONNECTED				= 0;
  static final int DEFAULT_CONNECTED			= 2;
  static final int SWITCH_CONNECTED			= 1;
  static final int FORWARD_DEFAULT			= 3;
  static final int LARGE_NUMBER				= 1000000;
  static final int NOTHING_DONE				= 0;


  /******************************************************************************
   * this is the best answer so far, we initialize it to an insanely large #
   ******************************************************************************/

  public static Integer best_so_far;
  ...
  public static void main(String [] args) throws Exception
  {
    ...
    BufferedReader infile = new BufferedReader(
                                               new FileReader(new File("cymbal.in")));
    ...
    int [][] connectivity = new int[MAX_CASE][MAX_CASE];
    ...
    int [] cur_answer = new int[MAX_CASE];
    int [] best_answer = new int[MAX_CASE];
    ...
    total_systems = Integer.parseInt(infile.readLine());
    ...
    for(int t_count = 1; t_count <= total_systems; t_count++)
    {
      ...
      best_so_far = new Integer(LARGE_NUMBER);
      ...
      total_switches = new Integer(infile.readLine());

      for(int s_count = 1; s_count <= total_switches.intValue(); s_count++)
      {
        ...
        default_setting = Integer.parseInt(tokenizer.nextToken());
        how_many_connections = Integer.parseInt(tokenizer.nextToken());
      ...
      System.out.print("Track System " + t_count + ":\n");
        ...
        if(temp_val > 1)
        {
          ...
          if( (connectivity[last_switch][current_switch] != 
              ...
              (connectivity[last_switch][current_switch] != 
               FORWARD_DEFAULT) )
          {
            System.out.print("(" + current_switch + ")");
          }
  ...
  public static int get_starting(int connectivity[][], Integer num_switches)
  {
    for(int to = 1; to <= num_switches.intValue(); to++)
    {
      int counter = 0;

      for(int from = 1; from <= num_switches.intValue(); from++)
      {
  ...
  public static void solve_for_one_switch(int connectivity[][],
                                          Integer switches_used,
                                          int current_val,
                                          int current_switch,
                                          int last_switch,
                                          int answer[],
                                          ...
                                          int best_answer[])
  {
    ...
    int howmany = connects_to_x(connectivity, switches_used, 
                                current_switch);
    ...
    if(current_val >= best_so_far.intValue())
      return;
    ...
    if(howmany == 0)
    {
      ...
      best_so_far = new Integer(current_val);
    ...
    for(int to = 1; to <= switches_used.intValue(); to++)
    {
      ...
      {
        solve_for_one_switch(connectivity, switches_used, current_val,
                             to, current_switch, 
                             answer, answer_index, best_answer);
      }
    }
  ...
  public static int connects_to_x(int connectivity[][],
				  Integer switches_used,
				  int switch_num)
  {
    int count = 0;
  ...
  public static void array_copy(int to[], int from[], int up_to_n)
  {
    int i;
    for(i = 0; i <= up_to_n; i++)
      to[i] = from[i];

    for(i = i; i < MAX_CASE; i++)
      to[i] = 0;
  }
}
